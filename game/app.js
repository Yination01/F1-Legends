...
function raceHub(){
  ...
  return `${topBar()}${seriesBar()}<div class="main-content simple-hub"><div style="padding:12px;display:flex;flex-direction:column;gap:12px">
  ...
  <div style="background:var(--panel);border:1px solid var(--line);border-radius:12px;overflow:hidden">
    ...
    <div style="padding:12px">
      ...
      <div style="margin-top:12px"><div style="font-weight:800;font-size:11px;margin-bottom:6px">RACE TYPE — 7 TYPES + FULL WEEKEND</div><div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px">${raceTypes.map(rt=>`<div class="car-component-slot ${window._selectedRaceType===rt.id?"selected":""}" data-race-type="${rt.id}" style="position:static;width:auto;height:auto;padding:8px"><div style="font-size:16px">${rt.icon}</div><div style="font-size:9px;font-weight:800;margin-top:2px">${rt.name}</div><div style="font-size:8px;color:var(--muted)">${rt.laps} laps</div><div style="font-size:7px;color:var(--muted)">${rt.hasPractice?"FP+":""}${rt.hasQuali?"Q+":""}R</div></div>`).join("")}</div><div id="raceTypeDesc" style="margin-top:8px;background:var(--bg2);border-radius:6px;padding:8px;font-size:11px"><b>${rtDef.icon} ${rtDef.name}</b> — ${rtDef.desc}<br><span style="font-size:10px">Laps: ${rtDef.laps} · Points: ${rtDef.points.join(", ")||"None"} · Tyre Rule: ${rtDef.tyreRule?"Yes":"No"} · DRS: ${rtDef.drs?"Yes":"No"} · Weekend: ${rtDef.hasPractice?"FP1/FP2/FP3 + ":""}${rtDef.hasQuali?"Q1/Q2/Q3 + ":""}Race · Simple race · </span></div></div>
      <button class="btn-clash btn-red" id="playRaceBtn" style="margin-top:12px">🏁 ${rtDef.icon} START WEEKEND: ${rtDef.hasPractice?"FP1 → ":""}${rtDef.hasQuali?"Q1/Q2/Q3 → ":""}${rtDef.name.toUpperCase()} — ${track.name} — Simple race</button>
    </div>
  </div>
...
function weekendHubScreen(track, myTeam, ghost, probs, raceTypeId){
  ...
  return `<div class="race-screen simple-race"><div class="race-topbar"><div class="lap-counter">${track.flag} ${track.name} · ${rt.icon} ${rt.name} Weekend · ${Team.getCurrentSlotId?Team.getCurrentSlotId():""}</div><div class="weather-widget">Setup +${totalSetup} · Simple race</div></div><div class="main-content" style="padding:12px;display:flex;flex-direction:column;gap:12px">
    <div style="background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:12px"><div style="font-weight:900;font-size:14px">🏁 RACE WEEKEND ${T.teamName}</div><div style="font-size:11px;color:var(--muted);margin-top:4px">FP1 → FP2 → FP3 → Q1/Q2/Q3 → Race. Setup bonus carries. simple race in race.</div><div style="margin-top:8px;display:flex;gap:6px"><span class="badge" style="background:var(--panel2)">FP1 ${weekend.practiceResults.fp1?"✓":"○"}</span><span class="badge" style="background:var(--panel2)">FP2 ${weekend.practiceResults.fp2?"✓":"○"}</span><span class="badge" style="background:var(--panel2)">FP3 ${weekend.practiceResults.fp3?"✓":"○"}</span><span class="badge" style="background:var(--panel2)">Q1/Q2/Q3 ${weekend.qualiResult?"✓":"○"}</span><span class="badge" style="background:var(--red);color:white">Race Simple race</span></div></div>
...
function qualifyingKnockoutScreen(track, myTeam, ghost, raceTypeId){
  ...
  return `<div class="race-screen simple-race"><div class="race-topbar"><div class="lap-counter">${track.flag} ${track.name} · ${rt.icon} ${rt.name} · Q1/Q2/Q3 · ${Team.getCurrentSlotId?Team.getCurrentSlotId():""}</div><div class="weather-widget">Pole ${qualiResult.pole.time.toFixed(3)}s</div></div><div class="main-content" style="padding:12px;display:flex;flex-direction:column;gap:12px">
    <div style="background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:12px"><div style="font-weight:900;font-size:14px">🎯 QUALIFYING — Q1/Q2/Q3 KNOCKOUT</div><div style="display:flex;gap:6px;margin-top:10px">${sessionTabs}<div class="car-component-slot" data-quali-session="FINAL" style="position:static;width:auto;height:auto;padding:8px;cursor:pointer;background:var(--red);color:white"><div style="font-weight:900;font-size:11px">FINAL GRID</div></div></div></div>${sessionContents}<div data-quali-content="FINAL" style="display:none"><div style="background:var(--panel);border:1px solid var(--line);border-radius:12px;overflow:hidden;margin-top:12px"><div style="padding:10px;font-weight:900;font-size:12px;background:var(--bg2);border-bottom:1px solid var(--line)">FINAL GRID — 20 CARS — POLE ${qualiResult.pole.name}</div>${finalGridRows}</div></div><button class="btn-clash btn-red" id="goRaceBtn">🏁 CONTINUE TO TYRE & BOOST →</button></div></div>`;
}
...
function labScreen(){
  ...
  return `${topBar()}<div class="main-content"><div style="padding:12px"><h3 style="font-weight:900">RESEARCH LAB ${T.teamName}</h3><h4 style="font-weight:800;font-size:12px;margin:12px 0 8px">🔧 COMPONENTS</h4>${compRows}<h4 style="font-weight:800;font-size:12px;margin:16px 0 8px">👥 DRIVERS ACADEMY</h4>${driverRows}</div></div>${bottomNav("lab")}`;
}
function marketScreen(){
  ...
  return `${topBar()}<div class="main-content"><div style="padding:12px"><h3 style="font-weight:900">MARKET ${Team.T.teamName}</h3>${rows}</div></div>${bottomNav("market")}`;
}
...
function moreScreen(){
  ...
  return `${topBar()}<div class="main-content"><div style="padding:12px;display:flex;flex-direction:column;gap:10px">
    <h3 style="font-weight:900">ALL FEATURES — </h3>
    ...
    <div style="background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:12px"><div style="font-weight:800;font-size:12px"></div><div style="font-size:11px;color:var(--muted);margin-top:6px">
...
function boostsScreen(){
  ...
  return `${topBar()}<div class="main-content"><div style="padding:12px"><div style="display:flex;justify-content:space-between;align-items:center"><h3 style="font-weight:900">🚀 BOOSTS ${T.teamName}</h3><button class="btn-clash btn-dark" id="backMore" style="width:auto;padding:6px 12px;font-size:11px">← MORE</button></div>${drivers}<h4 style="font-weight:800;font-size:12px;margin:16px 0 8px">INVENTORY</h4>${inventory}</div></div>${bottomNav("more")}`;
}
...
function spinnerScreen(){
  ...
  return `${topBar()}<div class="main-content"><div style="padding:12px"><div style="display:flex;justify-content:space-between;align-items:center"><h3 style="font-weight:900">🎰 SPINNER</h3><button class="btn-clash btn-dark" id="backMore5" style="width:auto;padding:6px 12px;font-size:11px">← MORE</button></div><div style="background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:20px;margin-top:12px;text-align:center"><div style="font-size:64px">🎰</div><button class="btn-clash ${canSpin?"btn-gold":"btn-dark"}" id="spinBtn" style="margin-top:16px">${canSpin?"🎰 SPIN NOW!":"⏰ WAIT 8 HOURS"}</button></div></div></div>${bottomNav("more")}`;
}
...
function tyreAndBoostSelection(track, myTeam, ghost, probs, quali, raceType){
  ...
  return `<div class="race-screen simple-race"><div class="race-topbar"><div class="lap-counter">TYRE & BOOST — ${Engine.RACE_TYPES[raceType].name}</div></div><div class="main-content" style="padding:12px;display:flex;flex-direction:column;gap:12px">
    ...
    <button class="btn-clash btn-red" id="confirmTyresBtn">🏁 START ${Engine.RACE_TYPES[raceType].name.toUpperCase()} → SIMPLE RACE</button>
  </div></div>`;
}
...
function raceScreen(track, myTeam, ghost, probs, quali, raceType){
  ...
  setTimeout(()=>{
    ...
    function drawTVCamera(positions){
      ...
      ctx.fillStyle="white"; ctx.font="bold 12px sans-serif"; ctx.fillText(`📺 TV — ${track.name} — LAP ${race.state.lap}/${track.laps}`, 10, 18);
    }
    function drawChaseCamera(positions){
      ...
      ctx.fillStyle="white"; ctx.font="bold 12px sans-serif"; ctx.textAlign="left"; ctx.fillText(`🎥 CHASE — Behind ${playerCar.name} P${playerCar.pos}`, 10, 18);
    }
    function drawOnboardCamera(positions){
      ...
      ctx.fillStyle="white"; ctx.font="bold 12px sans-serif"; ctx.fillText(`👁️ ONBOARD — ${playerCar.name} ${Team.T.teamName}`, 10, 18);
    }
    function drawHelicopterCamera(positions){
      ...
      ctx.fillStyle="white"; ctx.font="bold 12px sans-serif"; ctx.textAlign="left"; ctx.fillText(`🚁 HELICOPTER — ${track.name} — All 20 cars`, 10, 18);
    }
  ...
  <div style="background:var(--bg2);padding:6px 10px;font-size:10px;color:var(--muted);border-bottom:1px solid var(--line)">Strategy view ${Team.T.teamName}</div>
