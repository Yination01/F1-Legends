"use strict";
// F1 ZERO v5.2 — MULTIPLE SAVE SLOTS (TEMP LOCAL, WILL BE REPLACED BY GOOGLE PLAY SAVES) + 6 CAMERAS
...
function weekendHubScreen(track, myTeam, ghost, probs, raceTypeId){
  ...
  return `<div class="race-screen"><div class="race-topbar"><div class="lap-counter">${track.flag} ${track.name} · ${rt.icon} ${rt.name} Weekend · ${Team.getCurrentSlotId?Team.getCurrentSlotId():""}</div><div class="weather-widget">Setup +${totalSetup} · 6 CAMS</div></div><div class="main-content" style="padding:12px;display:flex;flex-direction:column;gap:12px">
    <div style="background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:12px"><div style="font-weight:900;font-size:14px">🏁 RACE WEEKEND — ${Team.getCurrentSlotId?Team.getCurrentSlotId():""} ${T.teamName}</div><div style="font-size:11px;color:var(--muted);margin-top:4px">FP1 → FP2 → FP3 → Q1/Q2/Q3 → Race. Setup bonus carries. 6 cameras in race.</div><div style="margin-top:8px;display:flex;gap:6px"><span class="badge" style="background:var(--panel2)">FP1 ${weekend.practiceResults.fp1?"✓":"○"}</span><span class="badge" style="background:var(--panel2)">FP2 ${weekend.practiceResults.fp2?"✓":"○"}</span><span class="badge" style="background:var(--panel2)">FP3 ${weekend.practiceResults.fp3?"✓":"○"}</span><span class="badge" style="background:var(--panel2)">Q1/Q2/Q3 ${weekend.qualiResult?"✓":"○"}</span><span class="badge" style="background:var(--red);color:white">Race 6 CAMS</span></div></div>
...
function practiceScreen(track, myTeam, sessionId, raceTypeId, ghost, probs){
  ...
  return `<div class="race-screen"><div class="race-topbar"><div class="lap-counter">${session.icon} ${session.name} — ${track.name} · ${Team.getCurrentSlotId?Team.getCurrentSlotId():""}</div><div class="weather-widget">${result.laps} laps</div></div><div class="main-content" style="padding:12px;display:flex;flex-direction:column;gap:12px">
...
function qualifyingKnockoutScreen(track, myTeam, ghost, raceTypeId){
  ...
  return `<div class="race-screen"><div class="race-topbar"><div class="lap-counter">${track.flag} ${track.name} · ${rt.icon} ${rt.name} · Q1/Q2/Q3 · ${Team.getCurrentSlotId?Team.getCurrentSlotId():""}</div><div class="weather-widget">Pole ${qualiResult.pole.time.toFixed(3)}s</div></div><div class="main-content" style="padding:12px;display:flex;flex-direction:column;gap:12px">
...
function moreScreen(){
  ...
  return `${topBar()}<div class="main-content"><div style="padding:12px;display:flex;flex-direction:column;gap:10px">
    <h3 style="font-weight:900">ALL FEATURES — V5.2 MULTI-ACCOUNT + 6 CAMERAS</h3>
...
<b>V5.1:</b> 6 cameras: Full Track (map) + Race Cam (F1 Clash isometric) + TV + Chase + Onboard + Helicopter, auto cam ON.<br>
...
function tyreAndBoostSelection(track, myTeam, ghost, probs, quali, raceType){
  ...
  return `<div class="race-screen"><div class="race-topbar"><div class="lap-counter">TYRE & BOOST — ${Engine.RACE_TYPES[raceType].name} — ${Team.getCurrentSlotId?Team.getCurrentSlotId():""}</div></div><div class="main-content" style="padding:12px;display:flex;flex-direction:column;gap:12px">
    ...
    <button class="btn-clash btn-red" id="confirmTyresBtn">🏁 START ${Engine.RACE_TYPES[raceType].name.toUpperCase()} → 6 CAMERAS — ${Team.getCurrentSlotId?Team.getCurrentSlotId():""}</button>
  </div></div>`;
}

/* ===== RACE SCREEN — WITH 6 CAMERAS LIKE F1 CLASH ===== */
function raceScreen(track, myTeam, ghost, probs, quali, raceType){
  ...
  setTimeout(()=>{
    ...
    function drawTrack(positions){
      if(cameraMode==="full") drawFullTrack(positions);
      else if(cameraMode==="tv") drawTVCamera(positions);
      else if(cameraMode==="race") drawRaceCamera(positions);
      else if(cameraMode==="chase") drawChaseCamera(positions);
      else if(cameraMode==="onboard") drawOnboardCamera(positions);
      else if(cameraMode==="helicopter") drawHelicopterCamera(positions);
      else drawRaceCamera(positions);
    }
    ...
    $$("[data-camera]").forEach(btn=>btn.onclick=()=>{ cameraMode=btn.dataset.camera; cameraAuto=false; updateCameraButtons(); toast(`🎥 ${CAMERAS[cameraMode].name}`); });
    $("#autoCamBtn").onclick=()=>{ cameraAuto=!cameraAuto; $("#autoCamBtn").textContent=cameraAuto?"AUTO CAM: ON":"AUTO CAM: OFF"; $("#autoCamBtn").style.background=cameraAuto?"var(--green)":"var(--panel2)"; };
    updateCameraButtons();
  ...
  return `<div class="race-screen"><div class="race-topbar"><div class="race-top-left"><div class="lap-counter" id="lapCounter">LAP 1/${track.laps} ${Engine.RACE_TYPES[raceType].icon} ${Engine.RACE_TYPES[raceType].name} · ${Team.getCurrentSlotId?Team.getCurrentSlotId():""}</div><div class="safety-car-banner hidden" id="scBanner">SAFETY CAR</div></div><div style="display:flex;gap:6px;align-items:center"><div class="weather-widget"><span class="weather-icon">☀️</span><span>22°C</span><span style="color:var(--muted)">Dry</span></div><div style="display:flex;gap:4px"><button class="pace-btn active-standard" data-speed="1" style="padding:4px 8px;font-size:10px">1x</button><button class="pace-btn" data-speed="2" style="padding:4px 8px;font-size:10px;background:var(--medium);color:#000">2x</button><button class="pace-btn" data-speed="3" style="padding:4px 8px;font-size:10px">3x</button></div></div></div>
  <div style="background:var(--panel);border-bottom:1px solid var(--line);padding:8px;display:flex;gap:6px;align-items:center;overflow-x:auto">
    <span style="font-weight:900;font-size:10px;white-space:nowrap">🎥 CAMERAS:</span>
    <button class="pace-btn" data-camera="full" style="padding:6px 10px;font-size:10px;white-space:nowrap">🗺️ Full Track</button>
    <button class="pace-btn" data-camera="race" style="padding:6px 10px;font-size:10px;white-space:nowrap;background:var(--red);color:white">🏎️ Race Cam</button>
    <button class="pace-btn" data-camera="tv" style="padding:6px 10px;font-size:10px;white-space:nowrap">📺 TV</button>
    <button class="pace-btn" data-camera="chase" style="padding:6px 10px;font-size:10px;white-space:nowrap">🎥 Chase</button>
    <button class="pace-btn" data-camera="onboard" style="padding:6px 10px;font-size:10px;white-space:nowrap">👁️ Onboard</button>
    <button class="pace-btn" data-camera="helicopter" style="padding:6px 10px;font-size:10px;white-space:nowrap">🚁 Heli</button>
    <button class="pace-btn" id="autoCamBtn" style="padding:6px 10px;font-size:10px;white-space:nowrap;background:var(--green);color:black">AUTO CAM: ON</button>
  </div>
  <div style="background:var(--bg2);padding:6px 10px;font-size:10px;color:var(--muted);border-bottom:1px solid var(--line)"><span id="cameraDesc">F1 Clash main race view</span> — Slot ${Team.getCurrentSlotId?Team.getCurrentSlotId():""} ${Team.T.teamName}</div>
